...
(define (pow b n)
  (if (= n 1) b
    (if (even? n) (pow (square b) (/ n 2)) (* b (pow (square b) (/ (- n 1) 2))) )))
...
(define (derive-product exp var)
  (make-sum (make-product (multiplier exp) (derive (multiplicand exp) var)) (make-product (derive (multiplier exp) var) (multiplicand exp))))
...
(define (make-exponentiation base exponent)
  (cond
    ((= exponent 0) 1)
    ((= exponent 1) base)
    ((number? base) and (number? exponent) (pow base exponent))
...
(define (exponentiation? exp)
    (cond
     ((pair? exp) and (= (length exp) 3) and (equal? (car exp) '^) #t)
     (else #f)))
