...
def do_lambda_form(vals, env):
    ...
    if len(vals) > 2:
        for parameter in vals.second.second:
            body = Pair(body, Pair(parameter, nil))
...
def do_define_form(vals, env): 
    ...
    if scheme_symbolp(target): 
        ...
        env.bindings[function_name] = do_lambda_form(Pair(args_list, body),env) 
...
def do_or_form(vals, env):
    ...
    eval_value = None
    for expr in vals:
        eval_value = scheme_eval(expr, env)
        if scheme_true(eval_value):
            return quote(eval_value)
    return expr
    ...
    for i, clause in enumerate(vals): 
        ...
        if scheme_true(test): 
            if len(clause) == 1:
                ...
                body = Pair('begin', helper_pairs(1, clause))
                return body
    return okay
