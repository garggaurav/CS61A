...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        return Calc_bacon(opponent_score)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    while i<=10:
        res = averaged_roll_dice(i, dice)
        if res>maxim_v:
            maxim = i
            maxim_v = res
        print(str(i)+" dice scores "+str(res)+" on average")
        i = i+1
...
def bacon_strategy(score, opponent_score):
    ...
    0
    """
    if Calc_bacon(opponent_score) >= BACON_MARGIN:
        return 0
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    bacon = Calc_bacon(opponent_score)
...
def final_strategy(score, opponent_score):
    """
    In order, following are the conditions implemented to increase win rate

    Use swap_strategy

    If rolling a 4 sided dice, use a custom stategy to minimize losses due to 4 sided dice

    Try to force a four sided dice on the opponent by using Free bacon rule

    If just 1 point away from a beneficial swap, roll 10 die to maximize chances of getting a 1

    If just 1 point away from a harmful swap, roll 1 dice to minimize chances of getting a 1
    """
    
    #result stores the strategy's decision
    fin = swap_strategy(score, opponent_score)
    bacon = Calc_bacon(opponent_score)
    #If rolling a 4 sided dice, check if using free_bacon gives more than 2 points, if so, take them, otherwise roll 4
    if (score+opponent_score)%7 == 0:
      if bacon>2:
        fin = 0
      else:
    ...
    #try to trap the opponent on a four sided dice using free bacon rule
    if (score + bacon + opponent_score + 1) % 7 == 0:
        fin = 0

    #try to trap the opponent on a four sided dice by rolling 10 dice, either a pig out occurs and swapping occurs or a high score is achieve.
    if (score+1)*2 == opponent_score or ((score+1)*2 == opponent_score + 1):
        fin = 10

    #try to avoid a harmful swap if only one point away from it by minimising the chances of scoring only one point
    if((opponent_score*2) - 1) == score:
        fin = 1
    
    return fin # this returnns the strategy's final answer

def Calc_bacon(opponent_score):
